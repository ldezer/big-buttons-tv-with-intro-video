  ...
  return (
    <ScreenContainer containerClassName="bg-white" edges={['top','left','right','bottom']}>
      <View style={styles.header}>
        <Pressable style={({ pressed }) => [styles.headerBtn, pressed && styles.headerBtnPressed]} onPress={() => router.back()}><Text style={styles.headerBtnText}>Cancel</Text></Pressable>
        <Text style={styles.headerTitle}>{existing ? 'Edit Profile' : 'New Profile'}</Text>
        <Pressable style={({ pressed }) => [styles.headerBtn, styles.saveBtn, pressed && styles.saveBtnPressed]} onPress={handleSave}><Text style={styles.saveBtnText}>Save</Text></Pressable>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.field}><Text style={styles.label}>Avatar Emoji</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.emojiRow}>{PROFILE_EMOJIS.map(e => <Pressable key={e} style={({ pressed }) => [styles.emojiOption, emoji === e && styles.emojiOptionSelected, pressed && styles.emojiOptionPressed]} onPress={() => setEmoji(e)}><Text style={styles.emojiOptionText}>{e}</Text></Pressable>)}</ScrollView></View>

        <View style={styles.field}><Text style={styles.label}>Profile Color</Text><View style={styles.colorRow}>{PROFILE_COLORS.map(c => <Pressable key={c} style={({ pressed }) => [styles.colorOption, { backgroundColor: c }, color === c && styles.colorOptionSelected, pressed && styles.colorOptionPressed]} onPress={() => setColor(c)}>{color === c && <IconSymbol name="checkmark" size={18} color="#FFFFFF" />}</Pressable>)}</View></View>
        ...
        <View style={styles.field}><Text style={styles.label}>Bundled Banner Slot</Text><View style={styles.segmentWrap}>{PROFILE_BANNER_OPTIONS.map(option => <Pressable key={option.key || 'none'} style={({ pressed }) => [styles.segment, bannerBundledArtKey === option.key && styles.segmentActive, pressed && styles.segmentPressed]} onPress={() => setBannerBundledArtKey(option.key)}><Text style={[styles.segmentText, bannerBundledArtKey === option.key && styles.segmentTextActive]}>{option.label}</Text></Pressable>)}</View></View>
        ...
        <View style={styles.field}><Text style={styles.label}>Button Text Size</Text><View style={styles.segmentWrap}>{(['large','xlarge','xxlarge'] as FontSize[]).map(fs => <Pressable key={fs} style={({ pressed }) => [styles.segment, fontSize === fs && styles.segmentActive, pressed && styles.segmentPressed]} onPress={() => setFontSize(fs)}><Text style={[styles.segmentText, fontSize === fs && styles.segmentTextActive]}>{fs === 'large' ? 'Large' : fs === 'xlarge' ? 'X-Large' : 'XX-Large'}</Text></Pressable>)}</View></View>

        <View style={styles.field}><Text style={styles.label}>Button Columns</Text><View style={styles.segmentWrap}>{([1,2,3] as ColumnCount[]).map(c => <Pressable key={c} style={({ pressed }) => [styles.segment, columns === c && styles.segmentActive, pressed && styles.segmentPressed]} onPress={() => setColumns(c)}><Text style={[styles.segmentText, columns === c && styles.segmentTextActive]}>{c} {c === 1 ? 'Column' : 'Columns'}</Text></Pressable>)}</View></View>

        <View style={styles.field}><View style={styles.toggleRow}><View style={styles.toggleInfo}><Text style={styles.label}>Spoken Labels</Text><Text style={styles.toggleSubtitle}>Read button labels aloud when tapped</Text></View><Pressable style={({ pressed }) => [styles.toggle, spokenLabels && styles.toggleOn, pressed && styles.togglePressed]} onPress={() => setSpokenLabels(v => !v)}><View style={[styles.toggleThumb, spokenLabels && styles.toggleThumbOn]} /></Pressable></View></View>

        {existing && <Pressable style={({ pressed }) => [styles.deleteButton, pressed && styles.deleteButtonPressed]} onPress={handleDelete}><IconSymbol name="trash.fill" size={18} color="#D32F2F" /><Text style={styles.deleteButtonText}>Delete Profile</Text></Pressable>}
      </ScrollView>
    </ScreenContainer>
  );
}
...
const styles = StyleSheet.create({
  ...
  colorRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 }, colorOption: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center', borderWidth: 3, borderColor: 'transparent' }, colorOptionSelected: { borderColor: '#1A1A1A' }, colorOptionPressed: { opacity: 0.8 },
